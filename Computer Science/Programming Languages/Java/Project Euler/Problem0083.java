...
public class Problem0083 {
	...
	private static int[] step(int[][] matrix, int[][] exploration) {
		...
		if (bestStep[0] == matrix.length - 1 && bestStep[1] == matrix[bestStep[0]].length - 1) {
			result[0] = 1;
		}
	...
	private static int cost(int i, int j, int[][] matrix, int[][] exploration) {
		...
		if (j < matrix[i].length - 1 && exploration[i][j + 1] != 0) {
			fromRight = exploration[i][j + 1] + matrix[i][j];
		}
	...
	private static int[][] readMatrix() {
		...
		try {
			in = new BufferedReader(new FileReader("p083_matrix.txt"));
			String line = null;
			int row = 0;
